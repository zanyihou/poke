...
angular.module('app')
  ...
  .controller('MainCtrl', function($scope,$http) {
	...
	$scope.myClickChip = function(param,event){	
		chipFun(param,event);
		
     	//socket.emit("userclip",(param,event)); 
		
	}

	var chipFun = function(param, event){
		...
		var newSmallStakeEl = '<span style="position:fixed; top:'+myStakeTop+'px; left:'+myStakeLeft+'px" class="t'+clickTimes+' chip-item-small chip-'+(myStake/rate)+'"></span>';
		$(".chip-list").append(newSmallStakeEl);
			
		let targetLeft = $(event.target).offset().left + $(event.target).width()/4;
		let targetTop = $(event.target).offset().top + $(event.target).height()/4;
